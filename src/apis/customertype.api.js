...
import * as Extender from './extenders/customertype.apiext.js'

const moduleName = 'customertype'
const headerSectionName = 'header'
const headerTableName = 'public.customertype' 	
...
export default class extends Api {
	...
	// contoh: header-list
	//         header-open-data
	async init(body) { return await customertype_init(this, body) }

	// extender call
	async execute(body) { return await customertype_execute(this, body) }

	// header
	async headerList(body) { return await customertype_headerList(this, body) }
	async headerOpen(body) { return await customertype_headerOpen(this, body) }
	async headerUpdate(body) { return await customertype_headerUpdate(this, body)}
	async headerCreate(body) { return await customertype_headerCreate(this, body)}
	async headerDelete(body) { return await customertype_headerDelete(this, body) }

			
}	

// init module
async function customertype_init(self, body) {
	const req = self.req
	...
	try {
		...
		if (typeof Extender.customertype_init === 'function') {
			// export async function customertype_init(self, initialData) {}
			await Extender.customertype_init(self, initialData)
		}
...
async function customertype_execute(self, body) {
	const { fnName } = body
	...
	if (typeof Extender[fnName] === 'function') {
		// export async function [fnName](self, db, body, customertype_log) {}
		return await Extender[fnName](self, db, body, customertype_log)
	} else {
...
// data logging
async function customertype_log(self, body, startTime, tablename, id, action, data={}, remark='') {
	const { source } = body
	const req = self.req
...
async function customertype_headerList(self, body) {
	const tablename = headerTableName
	const { criteria={}, limit=0, offset=0, columns=[], sort={} } = body
	const searchMap = {
		searchtext: `customertype_id = \${searchtext} OR customertype_name ILIKE '%' || \${searchtext} || '%'`,
	};
...
async function customertype_headerOpen(self, body) {
	const tablename = headerTableName

	try {
		const { id } = body 
		const criteria = { customertype_id: id }
		const searchMap = { customertype_id: `customertype_id = \${customertype_id}`}
		const {whereClause, queryParams} = sqlUtil.createWhereClause(criteria, searchMap) 
		const sql = sqlUtil.createSqlSelect({
			tablename: tablename, 
			columns:[], 
...
async function customertype_headerCreate(self, body) {
	const { source='customertype', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// record log
			customertype_log(self, body, startTime, tablename, ret.customertype_id, 'CREATE', logMetadata)
...
async function customertype_headerUpdate(self, body) {
	const { source='customertype', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	const startTime = process.hrtime.bigint()
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// eksekusi update
			const cmd = sqlUtil.createUpdateCommand(tablename, data, ['customertype_id'])
			const ret = await cmd.execute(data)
			...
			// record log
			customertype_log(self, body, startTime, tablename, data.customertype_id, 'UPDATE')
...
async function customertype_headerDelete(self, body) {
	...
	try {
		...
		const deletedRow = await db.tx(async tx=>{
			...
			const dataToRemove = {customertype_id: id}
			...
			// hapus data header
			const cmd = sqlUtil.createDeleteCommand(tablename, ['customertype_id'])
			...
			// record log
			customertype_log(self, body, startTime, tablename, id, 'DELETE', logMetadata)
