...
const CurrentSectionId = Context.Sections.customertypeHeaderList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('customertypeHeaderList-tbl')
const pnl_search = document.getElementById('customertypeHeaderList-pnl_search')
const btn_gridload = new $fgta5.ActionButton('customertypeHeaderList-btn_gridload') 

export const Section = CurrentSection
export const SearchParams = {}

export async function init(self, args) {
	console.log('initializing customertypeHeaderList ...')
	...
	try {
		...
		const obj_searchtext = document.getElementById('customertypeHeaderList-txt_searchtext')
		const btn_load = document.getElementById('customertypeHeaderList-btn_gridload')
		...
		// customertypeHeaderList-ext.mjs, export function initSearchParams(self, SearchParams) {} 
...
export function keyboardAction(self, actionName, evt) {
	if (actionName=='up') {
		...
		const customertypeHeaderEdit = self.Modules.customertypeHeaderEdit
		if (tbl.CurrentRow!=null) {
			customertypeHeaderEdit.Section.show({}, ()=>{
...
async function openRow(self, tr) {
	...
	const customertypeHeaderEdit = self.Modules.customertypeHeaderEdit
	customertypeHeaderEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await customertypeHeaderEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})

		// matikan atau nyalakan button prev/next sesuai kondisi
		setPagingButton(self, customertypeHeaderEdit)
...
function tbl_cellclick(self, evt) {
	...
	const customertypeHeaderEdit = self.Modules.customertypeHeaderEdit
	customertypeHeaderEdit.Section.show({}, (evt)=>{
		openRow(self, tr)
	})
...
async function tbl_loadData(self, params={}) {
	console.log('loading customertypeHeader data')
	console.log(params)
