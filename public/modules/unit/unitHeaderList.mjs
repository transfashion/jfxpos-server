...
const CurrentSectionId = Context.Sections.unitHeaderList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('unitHeaderList-tbl')
const pnl_search = document.getElementById('unitHeaderList-pnl_search')
const btn_gridload = new $fgta5.ActionButton('unitHeaderList-btn_gridload') 

export const Section = CurrentSection
export const SearchParams = {}

export async function init(self, args) {
	console.log('initializing unitHeaderList ...')
	...
	try {
		...
		const obj_searchtext = document.getElementById('unitHeaderList-txt_searchtext')
		const btn_load = document.getElementById('unitHeaderList-btn_gridload')
		...
		// unitHeaderList-ext.mjs, export function initSearchParams(self, SearchParams) {} 
...
export function keyboardAction(self, actionName, evt) {
	if (actionName=='up') {
		...
		const unitHeaderEdit = self.Modules.unitHeaderEdit
		if (tbl.CurrentRow!=null) {
			unitHeaderEdit.Section.show({}, ()=>{
...
async function openRow(self, tr) {
	...
	const unitHeaderEdit = self.Modules.unitHeaderEdit
	unitHeaderEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await unitHeaderEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})

		// matikan atau nyalakan button prev/next sesuai kondisi
		setPagingButton(self, unitHeaderEdit)
...
function tbl_cellclick(self, evt) {
	...
	const unitHeaderEdit = self.Modules.unitHeaderEdit
	unitHeaderEdit.Section.show({}, (evt)=>{
		openRow(self, tr)
	})
...
async function tbl_loadData(self, params={}) {
	console.log('loading unitHeader data')
	console.log(params)
