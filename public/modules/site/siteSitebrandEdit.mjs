...
const CurrentSectionId = Context.Sections.siteSitebrandEdit
...
const btn_edit = new $fgta5.ActionButton('siteSitebrandEdit-btn_edit')
const btn_save = new $fgta5.ActionButton('siteSitebrandEdit-btn_save')
const btn_new = new $fgta5.ActionButton('siteSitebrandEdit-btn_new', 'siteSitebrand-addrow')
const btn_del = new $fgta5.ActionButton('siteSitebrandEdit-btn_delete', 'siteSitebrand-delrow')
const btn_reset = new $fgta5.ActionButton('siteSitebrandEdit-btn_reset')
const btn_prev = new $fgta5.ActionButton('siteSitebrandEdit-btn_prev')
const btn_next = new $fgta5.ActionButton('siteSitebrandEdit-btn_next')

const btn_recordstatus = document.getElementById('siteSitebrand-btn_recordstatus')
const btn_logs = document.getElementById('siteSitebrand-btn_logs')

const frm = new $fgta5.Form('siteSitebrandEdit-frm');
const obj_sitebrand_id = frm.Inputs['siteSitebrandEdit-obj_sitebrand_id']
const obj_brand_id = frm.Inputs['siteSitebrandEdit-obj_brand_id']
const obj_sitebrand_isdisabled = frm.Inputs['siteSitebrandEdit-obj_sitebrand_isdisabled']
const obj_site_id = frm.Inputs['siteSitebrandEdit-obj_site_id']	
...
export async function init(self, args) {
	...
	CurrentState.getHeaderForm = () => {
		const siteHeaderEdit = self.Modules.siteHeaderEdit
		const frmHeader = siteHeaderEdit.getHeaderForm()
		return frmHeader
	}
...
export async function openSelectedData(self, params) {
	...
	try {
		...
		if (!CurrentState.headerFormLocked) {
			const fn_iseditdisabled_name = 'siteSitebrandEdit_isEditDisabled'
		...
		// export function siteSitebrandEdit_formOpened(self, frm, CurrentState) {}
		const fn_formopened_name = 'siteSitebrandEdit_formOpened'
...
export function headerLocked(self) {
	...
	// export function siteSitebrandEdit_formLocked(self, frm, CurrentState) {}
	const fn_name = 'siteSitebrandEdit_formLocked'
...
export function headerUnlocked(self) {
	...
	// export function siteSitebrandEdit_formUnlocked(self, frm, CurrentState) {}
	const fn_name = 'siteSitebrandEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm, CurrentState)
	}	
...
async function openData(self, id) {
	const url = `/${Context.moduleName}/sitebrand-open`
	try {
...
async function createData(self, data, formData) {
	const url = `/${Context.moduleName}/sitebrand-create`
	try {
...
async function updateData(self, data, formData) {
	const url = `/${Context.moduleName}/sitebrand-update`
	try {
		const result = await Module.apiCall(url, { data, source: Source }, formData) 
		return result 
	} catch (err) {
...
async function deleteData(self, id) {
	const url = `/${Context.moduleName}/sitebrand-delete`
	try {
...
async function backToList(self, evt) {
	...
	if (goback) {
		frm.lock()
		const listId =  Context.Sections.siteSitebrandList
...
async function  frm_unlocked(self, evt) {
	...
	// Extender untuk event Unlocked
	// export function siteSitebrandEdit_formUnlocked(self, frm) {}
	const fn_name = 'siteSitebrandEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm)
...
async function btn_new_click(self, evt) {
	...
	const siteSitebrandList = self.Modules.siteSitebrandList
	const listsecid = siteSitebrandList.Section.Id
	const fromListSection = sourceSection===listsecid

	if (fromListSection) {
		console.log('tambahkan row baru')
		CurrentSection.setSectionReturn(siteSitebrandList.Section)
	...
	try {
		...
		// export async function siteSitebrandEdit_newData(self, datainit, frm, CurrentState) {}
		const fn_newdata_name = 'siteSitebrandEdit_newData'
		...
		if (fromListSection) {
			...
			self.Modules.siteSitebrandList.Section.show()
		}
	}
}
...
async function btn_save_click(self, evt) {
	...
	// Extender Autofill
	// export async function siteSitebrandEdit_autofill(self, frm) {}
	const fn_autofill_name = 'siteSitebrandEdit_autofill'
	...
	// Extender Saving
	// export async function siteSitebrandEdit_dataSaving(self, dataToSave, frm, args) {}
	const args = { cancelSave: false }
	const fn_datasaving_name = 'siteSitebrandEdit_dataSaving'
	...
	try {
		...
		// Extender Saving
		const fn_datasaved_name = 'siteSitebrandEdit_dataSaved'
		const fn_datasaved = Extender[fn_datasaved_name]
		if (typeof fn_datasaved === 'function') {
			// export async function siteSitebrandEdit_dataSaved(self, data, frm) {}
		...
		if (isNewData) {
			...
			self.Modules.siteSitebrandList.addNewRow(self, data)
		} else {
			console.log('update data baris yang dibuka')
			self.Modules.siteSitebrandList.updateCurrentRow(self, data)
		}
...
async function btn_del_click(self, evt) {
	...
	try {
		...
		// Extender Deleting
		// export async function siteSitebrandEdit_dataDeleting(self, id, args) {}
		const args = { cancelDelete: false }
		const fn_datadeleting_name = 'siteSitebrandEdit_dataDeleting'
		...
		// Extender Delete
		// export async function siteSitebrandEdit_dataDeleted(self, data) {}
		const fn_datadeleted_name = 'siteSitebrandEdit_dataDeleted'
		...
		self.Modules.siteSitebrandList.removeCurrentRow(self)
...
async function btn_prev_click(self, evt) {
	...
	self.Modules.siteSitebrandList.selectPreviousRow(self)
}

async function btn_next_click(self, evt) {
	console.log('btn_next_click')
	self.Modules.siteSitebrandList.selectNextRow(self)
}
...
async function btn_recordstatus_click(self, evt) {
	...
	pageHelper.openSection(self, 'fRecord-section', params, async ()=>{
		...
		try {
			...
			// melalui extender siteSitebrandEdit_addRecordInfo
			// export async function siteSitebrandEdit_addRecordInfo(self,  data) {}
			const fn_addrecordinfo_name = 'siteSitebrandEdit_addRecordInfo'
