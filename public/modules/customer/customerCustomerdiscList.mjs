...
const CurrentSectionId = Context.Sections.customerCustomerdiscList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('customerCustomerdiscList-tbl')

const btn_addrow = document.getElementById('customerCustomerdiscList-btn_addrow') // tidak perlu pakai action, karna action didefine di edit
const btn_delrow = new $fgta5.ActionButton('customerCustomerdiscList-btn_delrow')
...
export async function init(self, args) {
	...
	CurrentSection.addEventListener($fgta5.Section.EVT_BACKBUTTONCLICK, async (evt)=>{
		const sectionId =  Context.Sections.customerHeaderEdit
	...
	// export function customerCustomerdiscList_addTableEvents(self, tbl) {}
	const fn_addTableEvents_name = 'customerCustomerdiscList_addTableEvents'
	...
	// Extend list detil
	// export function customerCustomerdiscList_init(self) {}
	const fn_name = 'customerCustomerdiscList_init'
	const fn_customerCustomerdiscList_init = Extender[fn_name]
	if (typeof fn_customerCustomerdiscList_init === 'function') {
		fn_customerCustomerdiscList_init(self)
	}
...
export async function openList(self, params) {
	...
	// bisa di set pada Extender.customerCustomerdiscList_openList :  bisa menggunakan template untuk di embed ke header pada detil list
	// export function customerCustomerdiscList_openList(self, headerForm) {}
	const fn_name = 'customerCustomerdiscList_openList'
	const fn_customerCustomerdiscList_openList = Extender[fn_name]
	if (typeof fn_customerCustomerdiscList_openList === 'function') {
		fn_customerCustomerdiscList_openList(self, headerForm)
	}
	...
	const customerCustomerdiscEdit = self.Modules.customerCustomerdiscEdit
	const btn_addrow = customerCustomerdiscEdit.getCurrentState().Actions.newdata
...
export function keyboardAction(self, actionName) {
	if (actionName=='up') {
		...
		if (tbl.CurrentRow!=null) {
			customerCustomerdiscEdit.Section.show({}, (evt)=>{
...
async function openRow(self, tr) {
	...
	const customerCustomerdiscEdit = self.Modules.customerCustomerdiscEdit
	customerCustomerdiscEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await customerCustomerdiscEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})
	} catch (err) {
		console.error(err)
	...
	setPagingButton(self, customerCustomerdiscEdit)
}


async function listRows(self, criteria, offset, limit, sort) {
	const url = `/${Context.moduleName}/customerdisc-list`
	const evt = { url, limit }

	// export async function customerCustomerdiscList_dataLoad(self, criteria, sort, evt) {}
	const fn_dataLoad_name = 'customerCustomerdiscList_dataLoad'
...
async function deleteRows(self, data) {
	const url = `/${Context.moduleName}/customerdisc-delete-rows`
	try {
		
		const result = await Module.apiCall(url, { data }) 
		if (result.deleted) {

			// export async function customerCustomerdiscList_rowsDeleted(self, data) {}
			const fn_name = 'customerCustomerdiscList_rowsDeleted'
...
async function tbl_loadData(self, params={}) {
	...
	try {
		...
		// export async function customerCustomerdiscList_tableDataLoaded(self, tbl, result) {}
		const fn_name = 'customerCustomerdiscList_tableDataLoaded'
		const fn = Extender[fn_name]
		if (typeof fn === 'function') {
