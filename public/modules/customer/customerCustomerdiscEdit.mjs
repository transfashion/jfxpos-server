...
const Extender = Ext.extenderCustomerdisc ?? Ext


const Crsl =  Context.Crsl
const CurrentSectionId = Context.Sections.customerCustomerdiscEdit
const CurrentSection = Crsl.Items[CurrentSectionId]
const Source = Context.Source
const CurrentState = {}

const TitleWhenNew = 'New Discount'
const TitleWhenView = 'View Discount'
const TitleWhenEdit = 'Edit Discount'
const EditModeText = 'Edit'
const LockModeText = 'Lock'



const btn_edit = new $fgta5.ActionButton('customerCustomerdiscEdit-btn_edit')
const btn_save = new $fgta5.ActionButton('customerCustomerdiscEdit-btn_save')
const btn_new = new $fgta5.ActionButton('customerCustomerdiscEdit-btn_new', 'customerCustomerdisc-addrow')
const btn_del = new $fgta5.ActionButton('customerCustomerdiscEdit-btn_delete', 'customerCustomerdisc-delrow')
const btn_reset = new $fgta5.ActionButton('customerCustomerdiscEdit-btn_reset')
const btn_prev = new $fgta5.ActionButton('customerCustomerdiscEdit-btn_prev')
const btn_next = new $fgta5.ActionButton('customerCustomerdiscEdit-btn_next')

const btn_recordstatus = document.getElementById('customerCustomerdisc-btn_recordstatus')
const btn_logs = document.getElementById('customerCustomerdisc-btn_logs')

const frm = new $fgta5.Form('customerCustomerdiscEdit-frm');
const obj_customerdisc_id = frm.Inputs['customerCustomerdiscEdit-obj_customerdisc_id']
const obj_customerdisc_start = frm.Inputs['customerCustomerdiscEdit-obj_customerdisc_start']
const obj_customerdisc_end = frm.Inputs['customerCustomerdiscEdit-obj_customerdisc_end']
const obj_customerdisc_ref = frm.Inputs['customerCustomerdiscEdit-obj_customerdisc_ref']
const obj_customerdisc_percent = frm.Inputs['customerCustomerdiscEdit-obj_customerdisc_percent']
const obj_customerdisc_rebate = frm.Inputs['customerCustomerdiscEdit-obj_customerdisc_rebate']
const obj_customer_id = frm.Inputs['customerCustomerdiscEdit-obj_customer_id']	
...
export async function init(self, args) {
	...
	CurrentState.getHeaderForm = () => {
		const customerHeaderEdit = self.Modules.customerHeaderEdit
		const frmHeader = customerHeaderEdit.getHeaderForm()
		return frmHeader
	}
...
export async function openSelectedData(self, params) {
	...
	try {
		...
		const suspended = self.Modules.customerHeaderEdit.getCurrentState().Actions.edit.isSuspended()
		...
		if (!CurrentState.headerFormLocked) {
			const fn_iseditdisabled_name = 'customerCustomerdiscEdit_isEditDisabled'
		...
		// export function customerCustomerdiscEdit_formOpened(self, frm, CurrentState) {}
		const fn_formopened_name = 'customerCustomerdiscEdit_formOpened'
...
export function headerLocked(self) {
	...
	// export function customerCustomerdiscEdit_formLocked(self, frm, CurrentState) {}
	const fn_name = 'customerCustomerdiscEdit_formLocked'
...
export function headerUnlocked(self) {
	...
	// export function customerCustomerdiscEdit_formUnlocked(self, frm, CurrentState) {}
	const fn_name = 'customerCustomerdiscEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm, CurrentState)
	}	
...
async function openData(self, id) {
	const url = `/${Context.moduleName}/customerdisc-open`
	try {
...
async function createData(self, data, formData) {
	const url = `/${Context.moduleName}/customerdisc-create`
	try {
...
async function updateData(self, data, formData) {
	const url = `/${Context.moduleName}/customerdisc-update`
	try {
		const result = await Module.apiCall(url, { data, source: Source }, formData) 
		return result 
	} catch (err) {
...
async function deleteData(self, id) {
	const url = `/${Context.moduleName}/customerdisc-delete`
	try {
...
async function backToList(self, evt) {
	...
	if (goback) {
		frm.lock()
		const listId =  Context.Sections.customerCustomerdiscList
...
async function  frm_unlocked(self, evt) {
	...
	// Extender untuk event Unlocked
	// export function customerCustomerdiscEdit_formUnlocked(self, frm) {}
	const fn_name = 'customerCustomerdiscEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm)
...
async function btn_new_click(self, evt) {
	...
	const customerCustomerdiscList = self.Modules.customerCustomerdiscList
	const listsecid = customerCustomerdiscList.Section.Id
	const fromListSection = sourceSection===listsecid

	if (fromListSection) {
		console.log('tambahkan row baru')
		CurrentSection.setSectionReturn(customerCustomerdiscList.Section)
	...
	try {
		...
		const customer_id = header_pk.value

		// inisiasi data baru
		const datainit = {
			customer_id,
			customerdisc_start: new Date(),
			customerdisc_end: new Date(),
			customerdisc_percent: 0,
			customerdisc_rebate: 0,
		}
		...
		// export async function customerCustomerdiscEdit_newData(self, datainit, frm, CurrentState) {}
		const fn_newdata_name = 'customerCustomerdiscEdit_newData'
		...
		if (fromListSection) {
			...
			self.Modules.customerCustomerdiscList.Section.show()
		}
	}
}
...
async function btn_save_click(self, evt) {
	...
	// Extender Autofill
	// export async function customerCustomerdiscEdit_autofill(self, frm) {}
	const fn_autofill_name = 'customerCustomerdiscEdit_autofill'
	...
	// Extender Saving
	// export async function customerCustomerdiscEdit_dataSaving(self, dataToSave, frm, args) {}
	const args = { cancelSave: false }
	const fn_datasaving_name = 'customerCustomerdiscEdit_dataSaving'
	...
	try {
		...
		// Extender Saving
		const fn_datasaved_name = 'customerCustomerdiscEdit_dataSaved'
		const fn_datasaved = Extender[fn_datasaved_name]
		if (typeof fn_datasaved === 'function') {
			// export async function customerCustomerdiscEdit_dataSaved(self, data, frm) {}
		...
		if (isNewData) {
			...
			self.Modules.customerCustomerdiscList.addNewRow(self, data)
		} else {
			console.log('update data baris yang dibuka')
			self.Modules.customerCustomerdiscList.updateCurrentRow(self, data)
		}
...
async function btn_del_click(self, evt) {
	...
	try {
		...
		// Extender Deleting
		// export async function customerCustomerdiscEdit_dataDeleting(self, id, args) {}
		const args = { cancelDelete: false }
		const fn_datadeleting_name = 'customerCustomerdiscEdit_dataDeleting'
		...
		// Extender Delete
		// export async function customerCustomerdiscEdit_dataDeleted(self, data) {}
		const fn_datadeleted_name = 'customerCustomerdiscEdit_dataDeleted'
		...
		self.Modules.customerCustomerdiscList.removeCurrentRow(self)
...
async function btn_prev_click(self, evt) {
	...
	self.Modules.customerCustomerdiscList.selectPreviousRow(self)
}

async function btn_next_click(self, evt) {
	console.log('btn_next_click')
	self.Modules.customerCustomerdiscList.selectNextRow(self)
}
...
async function btn_recordstatus_click(self, evt) {
	...
	pageHelper.openSection(self, 'fRecord-section', params, async ()=>{
		...
		try {
			...
			// melalui extender customerCustomerdiscEdit_addRecordInfo
			// export async function customerCustomerdiscEdit_addRecordInfo(self,  data) {}
			const fn_addrecordinfo_name = 'customerCustomerdiscEdit_addRecordInfo'
