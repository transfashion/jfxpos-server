...
import * as Ext from './customer-ext.mjs'
...
const CurrentSectionId = Context.Sections.customerHeaderEdit
const CurrentSection = Crsl.Items[CurrentSectionId]
const Source = Context.Source
const CurrentState = {}

const TitleWhenNew = 'New customer'
const TitleWhenView = 'View customer'
const TitleWhenEdit = 'Edit customer'
const EditModeText = 'Edit'
const LockModeText = 'Lock'

const btn_edit = new $fgta5.ActionButton('customerHeaderEdit-btn_edit')
const btn_save = new $fgta5.ActionButton('customerHeaderEdit-btn_save')
const btn_new = new $fgta5.ActionButton('customerHeaderEdit-btn_new', 'customerHeader-new')
const btn_del = new $fgta5.ActionButton('customerHeaderEdit-btn_delete')
const btn_reset = new $fgta5.ActionButton('customerHeaderEdit-btn_reset')
const btn_prev = new $fgta5.ActionButton('customerHeaderEdit-btn_prev')
const btn_next = new $fgta5.ActionButton('customerHeaderEdit-btn_next')


const btn_recordstatus = document.getElementById('customerHeader-btn_recordstatus')
const btn_logs = document.getElementById('customerHeader-btn_logs')
const btn_about = document.getElementById('customerHeader-btn_about')

const frm = new $fgta5.Form('customerHeaderEdit-frm');
const obj_customer_id = frm.Inputs['customerHeaderEdit-obj_customer_id']
const obj_customer_name = frm.Inputs['customerHeaderEdit-obj_customer_name']
const obj_customertype_id = frm.Inputs['customerHeaderEdit-obj_customertype_id']
const obj_gender_id = frm.Inputs['customerHeaderEdit-obj_gender_id']
const obj_customer_birthdate = frm.Inputs['customerHeaderEdit-obj_customer_birthdate']
const obj_customer_hasbirthdate = frm.Inputs['customerHeaderEdit-obj_customer_hasbirthdate']	
...
export async function init(self, args) {
	console.log('initializing customerHeaderEdit ...')
	...
	// export async function customerHeaderEdit_init(self, CurrentState)
	const fn_init_name = 'customerHeaderEdit_init'
	...
	// Combobox: obj_customertype_id
	obj_customertype_id.addEventListener('selecting', async (evt)=>{
		
		evt.detail.CurrentState = CurrentState
		
		const fn_selecting_name = 'obj_customertype_id_selecting'
		const fn_selecting = Extender[fn_selecting_name]
		if (typeof fn_selecting === 'function') {
			// create function di Extender (jika perlu):
			// export async function obj_customertype_id_selecting(self, obj_customertype_id, frm, evt) {}
			fn_selecting(self, obj_customertype_id, frm, evt)
		} else {
			// default selecting
			const cbo = evt.detail.sender
			const dialog = evt.detail.dialog
			const searchtext = evt.detail.searchtext!=null ? evt.detail.searchtext : ''
			const url = 'customertype/header-list'
			const sort = {}
			const criteria = {
			...
			// export function obj_customertype_id_selecting_criteria(self, obj_customertype_id, frm, criteria, sort, evt) {}
			const fn_selecting_criteria_name = 'obj_customertype_id_selecting_criteria'
			const fn_selecting_criteria = Extender[fn_selecting_criteria_name]
			if (typeof fn_selecting_criteria === 'function') {
				fn_selecting_criteria(self, obj_customertype_id, frm, criteria, sort, evt)
			}
			...
			try {
				...
				for (var row of result.data) {
					evt.detail.addRow(row.customertype_id, row.customertype_name, row)
				}
	...
	// Combobox: obj_gender_id
	obj_gender_id.addEventListener('selecting', async (evt)=>{
		
		evt.detail.CurrentState = CurrentState
		
		const fn_selecting_name = 'obj_gender_id_selecting'
		const fn_selecting = Extender[fn_selecting_name]
		if (typeof fn_selecting === 'function') {
			// create function di Extender (jika perlu):
			// export async function obj_gender_id_selecting(self, obj_gender_id, frm, evt) {}
			fn_selecting(self, obj_gender_id, frm, evt)
		} else {
			// default selecting
			const cbo = evt.detail.sender
			const dialog = evt.detail.dialog
			const searchtext = evt.detail.searchtext!=null ? evt.detail.searchtext : ''
			const url = 'gender/header-list'
			const sort = {}
			const criteria = {
				searchtext: searchtext,
			}
			...
			// buat function di extender:
			// export function obj_gender_id_selecting_criteria(self, obj_gender_id, frm, criteria, sort, evt) {}
			const fn_selecting_criteria_name = 'obj_gender_id_selecting_criteria'
			const fn_selecting_criteria = Extender[fn_selecting_criteria_name]
			if (typeof fn_selecting_criteria === 'function') {
				fn_selecting_criteria(self, obj_gender_id, frm, criteria, sort, evt)
			}
			...
			try {
				...
				for (var row of result.data) {
					evt.detail.addRow(row.gender_id, row.gender_name, row)
				}
...
export async function openSelectedData(self, params) {
	...
	try {
		...
		// export async function customerHeaderEdit_isEditDisabled(self, data)
		const fn_iseditdisabled_name = 'customerHeaderEdit_isEditDisabled'
		...
		// export async function customerHeaderEdit_formOpened(self, frm, CurrentState)
		const fn_formopened_name = 'customerHeaderEdit_formOpened'
...
async function backToList(self, evt) {
	...
	if (goback) {
		frm.lock()
		const listId =  Context.Sections.customerHeaderList
...
async function  frm_locked(self, evt) {
	...
	// export function customerHeaderEdit_formLocked(self, frm, CurrentState) {}
	const fn_name = 'customerHeaderEdit_formLocked'
	...
	// trigger lock event di customerdisc
	self.Modules.customerCustomerdiscList.headerLocked(self)
	self.Modules.customerCustomerdiscEdit.headerLocked(self)
...
async function  frm_unlocked(self, evt) {
	...
	// export function customerHeaderEdit_formUnlocked(self, frm, CurrentState) {}
	const fn_name = 'customerHeaderEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm, CurrentState)
	}
	...
	// trigger unlock event di customerdisc
	self.Modules.customerCustomerdiscList.headerUnlocked(self)
	self.Modules.customerCustomerdiscEdit.headerUnlocked(self)	
...
async function btn_new_click(self, evt) {
	...
	const customerHeaderList = self.Modules.customerHeaderList
	const listsecid = customerHeaderList.Section.Id
	...
	try {
		...
		if (fromListSection) {
			...
			self.Modules.customerHeaderList.Section.show()
		}
	}
}
...
async function btn_save_click(self, evt) {
	...
	// Extender Autofill
	const fn_autofill_name = 'customerHeaderEdit_autofill'
	...
	// Extender Saving
	// export async function customerHeaderEdit_dataSaving(self, dataToSave, frm, args) {}
	const args = { cancelSave: false }
	const fn_datasaving_name = 'customerHeaderEdit_dataSaving'
	...
	try {
		...
		// Extender Saving
		const fn_datasaved_name = 'customerHeaderEdit_dataSaved'
		const fn_datasaved = Extender[fn_datasaved_name]
		if (typeof fn_datasaved === 'function') {
			// export async function customerHeaderEdit_dataSaved(self, data, frm) {}
		...
		if (isNewData) {
			...
			self.Modules.customerHeaderList.addNewRow(self, data)
		} else {
			console.log('update data baris yang dibuka')
			self.Modules.customerHeaderList.updateCurrentRow(self, data)
		}
...
async function btn_del_click(self, evt) {
	...
	try {
		...
		self.Modules.customerHeaderList.removeCurrentRow(self)
...
async function btn_prev_click(self, evt) {
	...
	self.Modules.customerHeaderList.selectPreviousRow(self)
}

async function btn_next_click(self, evt) {
	console.log('btn_next_click')
	self.Modules.customerHeaderList.selectNextRow(self)
}
...
async function btn_recordstatus_click(self, evt) {
	...
	pageHelper.openSection(self, 'fRecord-section', params, async ()=>{
		...
		try {
			...
			const fn_addrecordinfo_name = 'customerHeaderEdit_addRecordInfo'
...
async function btn_about_click(self, evt) {
	...
	pageHelper.openSection(self, 'fAbout-section', params, async ()=>{
		...
		AboutSection.Title = 'About customer'
