...
const CurrentSectionId = Context.Sections.customerHeaderList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('customerHeaderList-tbl')
const pnl_search = document.getElementById('customerHeaderList-pnl_search')
const btn_gridload = new $fgta5.ActionButton('customerHeaderList-btn_gridload') 

export const Section = CurrentSection
export const SearchParams = {}

export async function init(self, args) {
	console.log('initializing customerHeaderList ...')
	...
	try {
		...
		const obj_searchtext = document.getElementById('customerHeaderList-txt_searchtext')
		const btn_load = document.getElementById('customerHeaderList-btn_gridload')
		...
		// customerHeaderList-ext.mjs, export function initSearchParams(self, SearchParams) {} 
...
export function keyboardAction(self, actionName, evt) {
	if (actionName=='up') {
		...
		if (tbl.CurrentRow!=null) {
			customerHeaderEdit.Section.show({}, ()=>{
...
async function openRow(self, tr) {
	...
	const customerHeaderEdit = self.Modules.customerHeaderEdit
	customerHeaderEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await customerHeaderEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})

		// matikan atau nyalakan button prev/next sesuai kondisi
		setPagingButton(self, customerHeaderEdit)
...
function tbl_cellclick(self, evt) {
	...
	const customerHeaderEdit = self.Modules.customerHeaderEdit
	customerHeaderEdit.Section.show({}, (evt)=>{
		openRow(self, tr)
	})
...
async function tbl_loadData(self, params={}) {
	console.log('loading customerHeader data')
	console.log(params)
