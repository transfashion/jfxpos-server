...
const CurrentSectionId = Context.Sections.rolePermissionList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('rolePermissionList-tbl')

const btn_addrow = document.getElementById('rolePermissionList-btn_addrow') // tidak perlu pakai action, karna action didefine di edit
const btn_delrow = new $fgta5.ActionButton('rolePermissionList-btn_delrow')
...
export async function init(self, args) {
	...
	CurrentSection.addEventListener($fgta5.Section.EVT_BACKBUTTONCLICK, async (evt)=>{
		const sectionId =  Context.Sections.roleHeaderEdit
	...
	// export function rolePermissionList_addTableEvents(self, tbl) {}
	const fn_addTableEvents_name = 'rolePermissionList_addTableEvents'
	...
	// Extend list detil
	// export function rolePermissionList_init(self) {}
	const fn_name = 'rolePermissionList_init'
	const fn_rolePermissionList_init = Extender[fn_name]
	if (typeof fn_rolePermissionList_init === 'function') {
		fn_rolePermissionList_init(self)
	}
...
export async function openList(self, params) {
	...
	// bisa di set pada Extender.rolePermissionList_openList :  bisa menggunakan template untuk di embed ke header pada detil list
	// export function rolePermissionList_openList(self, headerForm) {}
	const fn_name = 'rolePermissionList_openList'
	const fn_rolePermissionList_openList = Extender[fn_name]
	if (typeof fn_rolePermissionList_openList === 'function') {
		fn_rolePermissionList_openList(self, headerForm)
	}
	...
	const rolePermissionEdit = self.Modules.rolePermissionEdit
	const btn_addrow = rolePermissionEdit.getCurrentState().Actions.newdata
...
export function keyboardAction(self, actionName) {
	if (actionName=='up') {
		...
		if (tbl.CurrentRow!=null) {
			rolePermissionEdit.Section.show({}, (evt)=>{
...
async function openRow(self, tr) {
	...
	const rolePermissionEdit = self.Modules.rolePermissionEdit
	rolePermissionEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await rolePermissionEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})
	} catch (err) {
		console.error(err)
	...
	setPagingButton(self, rolePermissionEdit)
}


async function listRows(self, criteria, offset, limit, sort) {
	const url = `/${Context.moduleName}/permission-list`
	const evt = { url, limit }

	// export async function rolePermissionList_dataLoad(self, criteria, sort, evt) {}
	const fn_dataLoad_name = 'rolePermissionList_dataLoad'
...
async function deleteRows(self, data) {
	const url = `/${Context.moduleName}/permission-delete-rows`
	try {
		
		const result = await Module.apiCall(url, { data }) 
		if (result.deleted) {

			// export async function rolePermissionList_rowsDeleted(self, data) {}
			const fn_name = 'rolePermissionList_rowsDeleted'
...
async function tbl_loadData(self, params={}) {
	...
	try {
		...
		// export async function rolePermissionList_tableDataLoaded(self, tbl, result) {}
		const fn_name = 'rolePermissionList_tableDataLoaded'
		const fn = Extender[fn_name]
		if (typeof fn === 'function') {
